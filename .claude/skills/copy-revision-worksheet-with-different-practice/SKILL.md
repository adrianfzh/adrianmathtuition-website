---
name: copy-revision-worksheet-with-different-practice
description: >
  COPY one of Adrian's EXISTING revision documents and append a fresh "Practice" section of
  real past-paper questions from the math Supabase question bank — the base is byte-cloned,
  nothing in it is re-rendered. Two kinds: "notes" (base = a notes-bank fragment from
  Dropbox/AdrianMathNotes/Practice/<folder>/notes_bank, i.e. Adrian's own formulas + Reminders)
  and "worked" (base = an existing "(With Worked Examples)" sheet from
  Dropbox/AdrianMathNotes/Revision). Trigger when Adrian wants a different practice set on a
  sheet he ALREADY has — "another practice set for my AM Circles worked sheet", "notes-plus-practice
  for S4 EM Matrices, 10 questions", "copy the Partial Fractions revision sheet with new practice" —
  and on the shorthand "crw" as the first word of a terse comma-separated request: "crw, am circles,
  worked", "crw am partial fractions notes 10", "crw jc integration worked" (fields in any order:
  folder or bank, topic, kind notes|worked, optional question count). NOT revision-worksheet
  (`rw`): that one BUILDS a new worked-examples sheet from the bank when no base exists — reach
  for it when Adrian asks for a revision worksheet on a topic he has no sheet for. NOT
  create-worksheet (authors bare questions from scratch).
---
...
come in any order and case doesn't matter: `crw` first, then the folder/bank, the topic, the
kind, and optionally a bare number for the question count.

| He types | You run |
|---|---|
| `crw, am circles, worked` | `--kind worked --folder AM --topic "Circles"` |
| `crw am partial fractions notes 10` | `--kind notes --bank S3_AM --topic "Partial Fractions" -n 10` |
| `crw, em matrices, notes` | `--kind notes --bank S4_EM --topic "Matrices"` |
