...
POOLED=0
if [ -d "$LOGINS_DIR" ] && [ -r "$PICKER" ] && { ls "$LOGINS_DIR"/*/.credentials.json >/dev/null 2>&1 || { [ "$(uname)" = Darwin ] && [ -f "$LOGINS_DIR/pool-here" ] && ls "$LOGINS_DIR"/*/account >/dev/null 2>&1; }; }; then POOLED=1; fi  # a Mac keeps the login in the keychain (the picker finds it by the dir hash) and pools ONLY when pool-here exists: a login shared with the worker must not be used here too — a renewed key logs the other holder out
plan_limit_active() {
