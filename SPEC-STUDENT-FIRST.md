...
4. **Review my mistakes** (two evenings, §7). **BUILT 17 Sep 2026** — picker + deck + the five-day band + the jump. **Pixel-exact since the same evening** where the page has the marker's ink layer: `JumpToMistake` fetches that page's layer SVG, measures the question's own marks (`g[data-q]` boxes) and lands on their centre with a band their height; a page without a layer falls back to the marker's region words.
5. **Desk: Mark a paper button + Settings drawer** (one evening). The old pages redirect. **BUILT 17 Sep 2026 (first half)** — ⚙ Settings drawer on the desk header (`components/MarkingSwitches.tsx`: Mac plan only · Science tab · slot accounts), ✍️ Mark a paper + 🖊 Desk buttons on the profile's Papers tab. The mark page keeps its own switch cards for now. `/admin/papers` redirects to the desk's Completed lane (same evening); `/admin/mark-paper` stays as the intake.
...
1. **Notes on a marked paper.** The student writes on the marked pages in the app. Their ink is saved as **their own layer** (`student_ink`, one row per page, strokes as the overlay stores them), drawn on top of the marked page images. **The marked copy itself is never changed.** On the paper page a switch "Show my notes / Hide my notes" toggles the layer, and "Clear my notes" empties it after a confirm. The download offers both: "Marked copy" (the original) and "Marked copy with my notes" (flattened on request). No "clear to see the original" is ever needed, because the original is always there underneath.
   **Built 18 Sep 2026 in addition:** Adrian's own notes on the same pages — a second layer (`student_ink` identity `adrian`), drawn from the read-only paper view he reaches from the profile, saved through an admin-only door, shown to the student as "Adrian's notes" (hide, never erase). Lesson annotations that the student takes home; never the marking.
...
## 13. Pencil view additions (17 Sep 2026, evening)
