...
Status: **Phase 1 BUILT 3 Sep 2026 (bank grounding). Phase 2 BUILT 7 Sep 2026 — the library lives in SUPABASE, not Dropbox** (Adrian: "is it better if the exam library is in supabase?" → yes): the private `paper-library` bucket + `paper_library` table, filled by `scripts/paper-library/index.mjs` from the bank's own `source_file` list (299 PDFs on 7 Sep; 314 PDF sources not on this Mac, 1,120 DOCX sources awaiting a Word export job); the bot's `lib/paper-library.js` attaches questions + solutions as signed URLs at enqueue and again before marking (`result_json.paper_match.library`). **The attached-paper re-run BUILT 10 Sep 2026** (Isabelle's AM 2024 P1, run `9e66d0b4`: eight photos of working, the library attached the paper, the bank held its 13 rows, and the stamp still said `source:'none'` / `no-printed-questions` while `grounding.source` said `attached` — two chips contradicting each other): when the student's pages carry no printed questions and a paper PDF is attached, `lib/bank-grounding fetchBankGroundTruth` calls the marker's `readAttachedPaperOpenings` (one Sonnet read of the attached PDF under the pre-pass's own "verbatim" contract — the library's PDFs are scans, there is no text layer to read) and re-runs the SAME fingerprint guard (`isMatch`, unchanged thresholds) against those openings. Lazy: nothing is read unless the bank holds rows for the key. A trusted match is stamped `source:'library'` (or `'attached'` when Adrian attached the paper), `trusted:true`, reason `fingerprint-from-attached-paper`, `attached:{paper, openings, read:'model'}`; the block used is the bank's (`grounding.source:'bank'`), and the saved scheme's fingerprint carries the attached paper's openings too, so a later student who photographs the booklet matches rung 3. What `trusted` means here: the attached paper and the bank agree on which paper this is — the student's pages had nothing to say, so the residual risk is the library attach's own (a file filed under the wrong key). A mismatch is stamped `attached-paper-mismatch` (an anomaly for Adrian, never smoothed over); a failed read `attached-paper-unread`; a PDF with no readable questions `attached-paper-no-questions`; nothing attached stays `no-printed-questions`, fail-closed. Not covered: the single-photo / failed pre-pass path (`no-page-prepass`). **Same day, from a read-only probe of Isabelle's stored row:** the library had attached the TYS *Solutions* book as the paper — `chooseRows` took whichever row the database returned first, the book for 10 of the 12 keys holding both files — so the re-run read 0 openings from it (`attached-paper-no-questions`); the landing had dropped `paper_match.library`; and the 24-hour signed links were dead. Fixed in the bot (commit after `ee29326`): a questions-only file beats the combined book for the paper slot (a solutions file beats it for the scheme slot), `attachFromLibrary` re-signs its own links on every enqueue/claim and moves a book-as-paper slot onto the questions file (`library.refreshed` says so; hand-attached files still never touched), and `logMarkingRun` carries `paper_match.library` through the landing. Verified read-only on the real `questions.pdf` + the 13 bank rows: 23 openings read, 18 shared (78%) → `source:'library'`, `trusted:true`; the Solutions book alone → `{"questions":[]}` from Sonnet, `attached-paper-no-questions`. Isabelle's released run was NOT re-marked (Adrian's call — a desk 🔁 re-mark now attaches the questions file and lands the stamp). Phase 3 (extraction hand-off + the Submit-page ask) not built. Original: **SPEC AGREED 3 Sep 2026, build in phases below.** Follows the 5-step recipe in
...
1. **"kassandra am tys 2021 p1"**, three pages of working with question numbers only (no printed questions). Key = `gce 2021 am p1`. Bank has 27 rows. Fingerprint from her pages is EMPTY (no printed openings) → guard fails → `source:'none'`, reason `no-printed-questions`. Desk chip: "Not grounded — pages carry no printed questions." (Today's manual fix, attaching the 2021 PDFs, is rung 1 and stays valid.) With Phase 2, the library hit still attaches the 2021 questions + solutions because attachment does not need the fingerprint — the MARKER then reads the printed questions from the attached paper PDF, and the guard is re-run against that (built 10 Sep 2026: `source:'library'`, `trusted:true`, reason `fingerprint-from-attached-paper`; desk chip "🔍 gce 2021 am p1 · 27 matched", title "Source: library"). Isabelle's AM 2024 P1 was the case that showed the re-run missing: `grounding.source:'attached'` beside `paper_match.source:'none'`, and the sheet gate reading `paper_match.source` alone held her sheet (fixed on the site 10 Sep 2026 to use `isGroundedRun`, before the re-run landed).
