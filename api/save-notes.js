module.exports = async function handler(req, res) {
  ...
  if (auth !== process.env.ADMIN_PASSWORD) return res.status(401).json({ error: 'Unauthorized' });
  ...
  if (req.method === 'GET') {
    ...
    if (slug) {
      ...
      const data = await airtableFetch(
        '?fields[]=Topic&fields[]=Level&fields[]=Slug&sort[0][field]=Topic&sort[0][direction]=asc'
      );
  ...
  if (req.method === 'POST') {
    ...
    if (existingRecord) {
      await airtableFetch(`/${existingRecord.id}`, {
        method: 'PATCH',
