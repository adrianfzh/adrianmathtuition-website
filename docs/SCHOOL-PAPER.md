...
| Key | Paper | State |
|---|---|---|
| `sngs-s1-eoy` | CHIJ St Nicholas Girls, Sec 1 End-of-Year, 2 h 15 min, 90 marks, 20 questions | first paper written 18 Sep 2026 — models: **Opus** authored + repaired, **Sonnet** blind-solved, **Fable** moderated, **Sonnet** drew the 7 figures (the Sec 1 split; the TJC IP4 papers of 20 Sep used Fable author/moderator, Opus solver/figures) (20 questions, 90 marks, 7 figures, all slots through blind solve and moderation), with Adrian for his read; files in `~/Desktop/AdrianMath/School Papers/St Nicholas S1 EOY/` |
| `sngs-s1-eoy-set2` | the same St Nicholas Sec 1 paper, SET 2, same difficulty (Adrian, 21 Sep 2026: "Vary the questions, and concepts tested. Similar difficulty") | WRITTEN 21 Sep 2026 — 20/20 accepted, 90 marks, 7 figures + the Q14 construction space, blind solve 130 min (15 same, 5 slightly harder). Its own slot plan (`schools/sngs-s1-eoy-set2/plan.json`) so the briefs are NOT Set 1's: each slot names a different skill inside the same topic, which is why only three slots needed repair (Q3 too close to 2024 Q7, Q8 marks mismatch, Q9 novelty 0.32 → a regular octagon with two sides produced in place of the planned pentagon). Models: Opus author + repair, Sonnet blind solve, Fable moderate, Sonnet figures. Lessons: (1) a construction question's working space is a hand-made `Q<n>.png` at a known px-per-cm, pinned by `figure-sizes.json` — measure the line in the finished PDF (fitz `get_image_info`) and tell Adrian to print at 100 %; (2) on a 3-D solid the engine's label placer can drop a height label onto a base edge, where it reads as that edge's length — view every figure and move it (smaller `fs` frees a position); (3) real agent cost was 92k–165k tokens each, not the ~25k estimated — quote that when announcing a round. Files in `~/Desktop/AdrianMath/School Papers/St Nicholas S1 EOY/set2/` |
...
### Maths the same size as the text (21 Sep 2026)

Adrian, reading the St Nicholas PDF on his phone: "the fonts are of different sizes?" Two causes, two fixes.

- The Word file: a maths run carried no size, so some readers drew it at 11 pt beside 9.5 pt text. `export-docx.py` `size_math()` now writes the body size on every maths run before saving.
- The LibreOffice PDF: LibreOffice draws every imported Word formula at its own 12 pt whatever the file says. Make the PDF with `scripts/gce-paper/lo-pdf.sh <file.docx>`, never a plain `soffice --convert-to pdf`. It sets every formula to 10 pt, then exports. A PDF made by Word itself needs none of this.
