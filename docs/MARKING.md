...
- **✍️ Write on my paper (17 Sep 2026, SPEC-STUDENT-FIRST §12, phase 1):** the paper page's marked pages render inside `StudentInk.tsx` with the student's ink as an SVG layer over each image; "Write on my paper" opens `components/AnnotateOverlay` in **student mode** (`mode="student"`: the mouse draws too, no flatten/assemble, its own draft key `student:<run>`, Save → `onSaveInk`) → `POST /api/portal/marking/ink {runId, pages}` upserts `student_ink` (run × identity; shape + caps in `lib/student-ink.ts`, pure/tested; empty = delete). "Hide my notes" shows the original, "Clear my notes" empties the layer. **The marked copy is never written.** Science papers keep the plain images. Health-check `marking-ink` probes the 401. Not yet: a download with the notes flattened in.
