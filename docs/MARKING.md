...
- **🔁 Re-mark, two doors** (row button 8 Sep 2026 — Adrian: "can you put a re-mark button? and where else can i re-mark?"): every marked row in the history list carries **🔁 Re-mark** beside Load (`remarkRun` → `markFromStored(run.id)`, `phase:'remark'` on the run's stored photos + paper PDF, confirm first, full marking cost, marking replaced IN PLACE on the same run id); the loaded run's own **🔁 Re-mark** above the PDF buttons is the same call. The desk's detail view links here (`?run=<id>` → "✍️ Re-mark on the marking page") — the desk itself never re-marks; its 🔁 New sheet rebuilds only the sheet. Nowhere else: triage and the papers library have no re-mark. A re-mark cancels any sheet job still being written and queues a new one (`autoQueueSheet` with `remark:true`).
