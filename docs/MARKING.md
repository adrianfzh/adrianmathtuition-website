...
- **🔁 Re-mark, two doors** (row button 8 Sep 2026 — Adrian: "can you put a re-mark button? and where else can i re-mark?"; then "why is the latest marked copy of denise pdf costing $3+?"): every marked row in the history list carries **🔁 Re-mark** beside Load — it goes THROUGH THE 🌙 QUEUE (`phase:'enqueue'` with `remark:true`): the bot moves the old marking aside (`result_json.previous_results`/`previous_totals`, totals nulled so the row reads as a fresh queued paper), forgets every saved page read (`external-reads.clearReads`), stamps `queue.remark`, and the Mac plan-marker claims it when a slot is awake (~$0.20, 3–4 min) or the API path takes it after the head start; on delivery `deliverQueuedRun` POSTs `/api/admin/sheet-jobs {runId, remark:true}` so the Practice Again sheet is rewritten (a released paper is refused there). The loaded run's **⚡ Re-mark now** above the PDF buttons is the direct API call (`phase:'remark'`, ~$3, 1–2 min) — it writes a NEW run beside the old one (Denise, 8 Sep 2026: 56/90 at 22:38 and 55/90 at 00:47, each with its own sheet and dated Dropbox folder; the one-mark drift is the known ±4/90 run-to-run variance); the bot marks the old run `superseded_by` the new and archives it when unreleased, dropping its held practice items, so the desk shows one row. The desk's detail view links here (`?run=<id>` → "✍️ Re-mark on the marking page") — the desk itself never re-marks; its 🔁 New sheet rebuilds only the sheet. Nowhere else: triage and the papers library have no re-mark. A re-mark cancels any sheet job still being written and queues a new one (`autoQueueSheet` with `remark:true`).
...
- **📄 The paper opens on its pages, write-anywhere (18 Sep 2026 — Adrian: "write on my paper is hard to use … a pdf shows up and they are able to annotate on the pdf directly"):** the paper page is two tabs (`marking/PaperTabs.tsx`, both mounted; `?view=marks` opens the second, a `?page=` jump always lands on the first): **My paper** = the marked pages as one continuous scroll, and **Where my marks went** = the cover + every dropped mark (`LostMarks`). Practice Again sits above both. On the pages there is no writing mode and no Save: the Pencil writes wherever it touches, a finger only scrolls, ink saves itself ~1.2 s after the last stroke (and at once on leaving the page), a sticky bar has Pen · Highlight · Erase · Undo · ☝️ Finger writes (one finger writes, two scroll — phones). `StudentInk.tsx` draws each page on a **`<canvas>`, never an `<img>`** (iPadOS Live Text swallows Pencil strokes over an `<img>`'s printed text — the 4 Aug lesson) and keeps only the bitmaps near the viewport; on Apple devices the Pencil is read **only from the stylus TOUCH stream** (`touchType 'stylus'`, `preventDefault` so it does not scroll; Safari drops Pencil pointer events mid-stroke), the pointer stream serves mouse and other pens. Pure pieces + tests: `lib/inline-ink.ts` (screen → page-pixel mapping, tool widths by page size, segment-distance eraser, tap/palm-graze rejection). Same table, same two layers, same routes as below; an inline save clears the full-screen overlay's old local draft so it cannot come back over newer ink. **Full screen ⤢** still opens the overlay (zoom, typed notes, shapes) on the same layer. A native app (PencilKit) is the only way to GoodNotes-grade feel — later, as an unlisted App Store app. **The pen, round two (22 Sep 2026, Adrian from the iPad):** the toolbar pill is pinned to the VISUAL viewport under a pinch zoom (`window.visualViewport` resize/scroll → `lib/inline-ink toolbarPlacement`, inline left/top + `scale(1/scale)` from the bottom-centre — a `position: fixed` element is fixed to the layout viewport and would zoom away with the page); three sizes **S / M / L** per tool (`InkSize`, `SIZE_FACTOR` on `toolWidth`, `ERASER_SCREEN_PX` → `eraserRadius`; `ink-size:<editor>:<tool>` in localStorage; a second tap on the active tool opens the popover — colours + sizes for pen and highlighter, sizes for the eraser); **the eraser is partial** (`eraseAt` cuts every touched freehand stroke with `annotate/stroke-split splitStrokeAtCircle`, a typed note goes whole, one undo step per erase gesture via the `phase:'start'|'move'` on `onErase`); twelve pen and eight highlighter colours; and draw-and-hold snaps a hand CIRCLE to a circle again — the hold cluster at the end, the overshoot past the start and the jitter used to break the ellipse fit, and the rectangle test then accepted the round loop's inscribed square (`shape-fit cleanLoop` trims clusters + cuts the overshoot + smooths, rect and ellipse are contested by `outlineResidual`, `fitRect` merges RDP vertices closer than 8 % of the perimeter into one rounded corner; regression cases in `shape-fit.test.ts` "a held stroke on the iPad"). **Later that day (Adrian: "trace a curve, then the pen stroke snaps to the closest fitted curve, like Notability"):** a held stroke that is no shape snaps anyway — an open stroke on a circle becomes a clean arc (`fitArc`, Kåsa circle, 25°–340°), everything else the smoothed curve the hand meant (`fitCurve` → `lib/annotate/curve-fit.ts`, Schneider's piecewise cubic Bézier fit within 2 % of the stroke's length); both stored as polylines with `snapped: 'arc' | 'curve'`, so the eraser, the lasso, the PDF bake and the student pen (`StudentInk.tsx`) take them unchanged; only a stroke under the minimum length keeps its freehand ink.
- **✍️ Write on my paper (17 Sep 2026, SPEC-STUDENT-FIRST §12, phase 1 — the full-screen path since 18 Sep):** the paper page's marked pages render inside `StudentInk.tsx` with the student's ink as an SVG layer over each image; "Write on my paper" opens `components/AnnotateOverlay` in **student mode** (`mode="student"`: the mouse draws too, no flatten/assemble, its own draft key `student:<run>`, Save → `onSaveInk`) → `POST /api/portal/marking/ink {runId, pages}` upserts `student_ink` (run × identity; shape + caps in `lib/student-ink.ts`, pure/tested; empty = delete). "Hide my notes" shows the original, "Clear my notes" empties the layer. **The marked copy is never written.** **Typed notes** (student mode's T tool) are text strokes on the same layer (`Stroke.text`, drawn by the canvas painter, `strokesToSvg`, the sheet's flatten and the notes PDF alike). **Triangle snap** joined line / rect / ellipse in `lib/annotate/shape-fit`. The mouse draws in student mode (laptops); **☝️ Finger draws / Finger scrolls** on the toolbar (student mode only, off by default) routes a single finger into the pen path for phones and pen-less tablets — while on, fingers do not scroll or zoom. Science papers keep the plain images. Health-check `marking-ink` probes the 401. **Download with notes** (same day): `GET /api/portal/marking-pdf?run=&kind=marked&notes=1` fetches the annotated page images, composites each inked page with its SVG strokes (sharp, viewBox = the ink's page size so a resized render still lines up) and assembles a 595pt-wide PDF with pdf-lib — built on request, never stored; the file is named "… (with my notes).pdf". The paper page's download row shows "⬇ With my notes" only when ink exists. **Adrian's notes (18 Sep 2026 — "i would like to draw on it to show annotations to students physically … save to student's app"):** a SECOND layer in the same table under identity `adrian` (`TEACHER_INK_IDENTITY`), written only through `POST /api/admin/marking/teacher-ink` (admin session/bearer). The paper page loads both: the student edits theirs and sees "Adrian's notes" read-only (hide, never erase); Adrian, opening the paper from the profile's Papers tab (his admin cookie; the page is otherwise read-only for him — no rename/star/archive/remark/clip/hand-in/request), edits HIS layer with "Write on their paper" and sees theirs. Draft keys differ (`adrian:<run>` vs `student:<run>`). The notes download composites Adrian's layer first, the student's on top. Nothing touches the marking, the PDF, the marks or the desk; no message is sent — the student is NOT notified of Adrian's ink and the app shows no dot for it (22 Sep 2026, asked and confirmed). **🖊 Admin mode on the student's paper (22 Sep 2026 — Adrian: "put the desk's machinery into student … just replace that expanded button to go into admin mode … i do not want the per-question agree/override UI"):** on his admin cookie the pen bar's ⤢ Full-screen button is **Edit marking** (`marking/AdminMarkingPen.tsx` wraps `StudentInk` with `onEditMarking`): it flushes his ink, loads the run from `GET /api/admin/desk/run?runId=` (the desk's own route, mapped exactly as the desk's `annotatePages`) and opens `AnnotateOverlay` in ADMIN mode at the page he was on — the editable marker layer of SPEC-ANNOTATE §14–15: move / delete / retype ticks, crosses, notes, verdicts; ✓⇄✗ swap (ink only, an `ink_hints` row, never a mark); a retyped **score chip** (`3/5`) IS the mark change — part → question total → `recomputeTotals`. Done composes the page on the server, then, because the paper is released, `POST mark-triage {action:'reissue'}` rebuilds the PDFs and cover and tells the student (Telegram where linked), and the page refreshes with the new images and total. No Agree/Override rows exist on this door; the desk keeps its own copy of the same overlay. **Four shortcuts (later on 22 Sep 2026, SPEC-ANNOTATE §16, admin mode only):** a selected score chip shows a 0…max number row (one tap = the mark), a quick tap on a ✓/✗ with the Marks tool flips it (hold or drag still moves it), the eraser rubs out the marker's own ticks / crosses / notes / boxes when it hits none of Adrian's ink, and red ✓ / ✗ stamp tools plant a mark in the marker's hand (`lib/annotate/layer.ts addMarkObject`). **🔗 Belongs to a paper (18 Sep 2026):** a Practice Again sheet that arrived with no link (a photo through the general Hand in page, a Telegram hand-in) shows as its own card with "belongs to a paper ▾" on Adrian's tab; `POST /api/admin/papers {runId, belongsTo}` writes one worksheet assignment row (source `adrian`, note `linked by Adrian`, run_id = the sheet, source_run_id = the paper) — the same link the automatic sheets carry, so the student's Papers page groups it too; `belongsTo: null` (the sheet line's "unlink") deletes only that row. Sheets Send-work sent from a paper (source `adrian` + `source_run_id`) group like Practice Again ones since the same day. **The desk's ⚙ Settings drawer** (`components/MarkingSwitches.tsx`) carries the three switches the mark page has, through the same two routes.
- **✍️ Do it in the app (17 Sep 2026, §12 use 2):** `/app/work/[assignment id]` — the sheet's PDF becomes page images in the browser (`lib/pdf-pages`, the hand-in's own rasteriser), the same overlay in student mode writes on them, progress saves to `student_work_ink` via `POST /api/portal/work/ink {assignmentId, pages, pageCount}` (409 once handed in), and **Submit** flattens each page (image + ink, `work-in-app.tsx drawStrokes`) → uploads through `/api/portal/submit-token` → `POST /api/portal/submit {photoUrls, paperName, assignmentId}` — the exact photo hand-in, so marking, auto-release and the assignment flip are untouched. Doors: "Start ›" on the Papers row, "Do it in the app" on the paper page's sheet card and on `/app/assignments/[id]`; the PDF and "Hand in a photo" stay. Health-check `work-ink` probes the 401.
- **My remark (17 Sep 2026, option 2 — Adrian reads it too):** a quiet grey box under the paper page's header (`PaperNote.tsx`, saves on a typing pause and on blur) → `POST /api/portal/marking/note {runId, note}` writes `paper_marking_runs.student_note` (rule `lib/paper-label normalizeStudentNote`, ≤ 600 chars, line breaks kept); the Papers row shows the first line in grey italics; Adrian's mirror shows "their remark:" under the paper. Health-check `marking-note` probes the 401.
- **🗂 Archive (17 Sep 2026):** "Archive" on the paper page → `POST /api/portal/marking/archive {runId, on}` writes `paper_marking_runs.student_archived_at`; the paper leaves the tab's list for a folded "Archived (n)" row at its foot with "Put back in my list"; it still counts in the tiles; the desk and the profile ignore the column. Health-check `marking-archive` probes the 401.
- **🔍 Search (17 Sep 2026):** `PaperSearch.tsx` (client) wraps each tab's entries — every row and bundle is rendered on the server and handed over with a haystack (names, dates, scores); the box appears once a tab holds `SEARCH_FROM` = 8 papers.
- **The full typeset report is no longer drawn (17 Sep 2026, Adrian: "no longer a need for full pdf … only image pdf"):** the bot's post-marking build and `lib/rebuild-run-pdfs rebuildModes` make the images copy alone when annotated pages exist, and the full report only as the fallback for a paper with none — one Puppeteer render and one upload fewer per paper. `result_json.results` (the typed transcript) is untouched: the cover page and the Notebook read it. Old runs keep their `pdf_url`; the desk's "Full" link shows only when one exists.
...
- **Pen-only ink** (`pointerType==='pen'`; mouse only behind `?mouse=1`) = palm
  ...
  2/3-finger tap = undo/redo. Draw-and-hold ≥500ms snaps line/rect/triangle/ellipse, and since 22 Sep 2026 an open stroke on a circle to an arc and anything else to a smoothed curve (SPEC-ANNOTATE §4).
