...
- **📝 My note (17 Sep 2026, option 2 — Adrian reads it too):** a light box under the paper page's header (`PaperNote.tsx`, saves on a typing pause and on blur) → `POST /api/portal/marking/note {runId, note}` writes `paper_marking_runs.student_note` (rule `lib/paper-label normalizeStudentNote`, ≤ 600 chars, line breaks kept); the Papers row shows the first line in amber italics; Adrian's mirror shows "their note:" under the paper. Health-check `marking-note` probes the 401.
