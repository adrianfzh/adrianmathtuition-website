...
- **Reschedule labels carry the weekday and the origin** (2026-07-24) — rescheduled-away sub-line reads `Rescheduled → Fri, 24 Jul 5-7pm`; the DESTINATION Rescheduled/Makeup chip shows `↩ from Fri, 24 Jul 3-5pm` (`rescheduledFrom` in the schedule payload: same-week source record reverse-lookup, else parsed from the `Rescheduled from …`/`Makeup for …` note the create routes stamp — cross-week sources with fully custom notes have no origin). **A move to an EARLIER date reads `Brought forward to Mon, 24 Aug 3-5pm ✓`** (2026-09-04) and, unlike every other moved-away chip, is drawn on its original date even while that date is still in the future — the slot card otherwise hides moved-away lessons on future dates as noise, and Klaire's 5 Sep lesson taught on 24 Aug looked like a missing record. Direction = `isBroughtForward(sourceDate, finalDate)` in `lib/reschedule-chain.ts` (tested); the rule lives in `renderLessonsSlotCard`'s visibility filter.
...
- **Chip info lines (no pill — retired 2026-07-26)** — the exam/topic pill is gone; the student NAME is the tap target for the Exam | Regular-work sheet. Chips show labelled sub-lines instead: `📅 WA3 · A Math · 20 Aug — topics` per subject (`examSummaryLines()`, exam type leads; P1/P2 split renders `P1 …, P2 …`), `✅ WA3 done` (green) once all exam dates are past, `📘 Working on: <topics>` + `📗 Next lesson: <topics>` (from `currentTopicByStudent`/`nextTopicByStudent`), muted `no exam / topic yet` when nothing is recorded, `📋 Project Work · no WA3` for PW/AA. **Topics collapse on every exam line** (EOY/Promo since 2026-08-26, ALL types since 2026-09-07): the chip line shows a blue `— N topics ▸` (or `details ▸` when only a note is saved) instead of the topic wall; tapping it expands topics + note in place (tap the expanded text to re-collapse; one flag per chip, `examDetailsOpen`). It was WA/Prelim-inline until a WA3 testing all of Sec 3 plus lower-sec review put 35 topics on one chip (Lucas, 7 Sep 2026). The exam sheet always shows everything.
...
- **Season handover — EOY info shows while WA3 is still the active season** (2026-08-22) — the schedule used to fetch ONLY the active season's Exams, so Jeanette/Adela's freshly entered EOY info stayed invisible (chips + dialog kept showing WA3) until the calendar flipped on 20 Sep. Now `scheduleExamTypes(active)` (lib/exam-season.ts, unit-tested) returns active-or-previous season + `nextExamType()` + Prelim + Promo, the route fetches all of them, and **`pickDisplaySeason(records, active, today)` chooses per student**: the season with the earliest exam still ahead (a date-less non-NoExam row = TBC = ahead) → else the active season as "done" → else the latest saved. `examSeasonByStudent[sid] = {examType, upcoming}` drives the chip (`📅 EOY · …`), the Exam-vs-Regular-work default tab (`upcoming` — keeps the 2026-07-25 "exams over → Regular work" rule), the dialog's default type (`defaultEditExamType`: student's upcoming season → it; season over → next; nothing saved → active, or next in a gap; then `levelSpecificExamType` maps Sec4/JC2→Prelim in WA3, JC1→Promo in EOY) and `lesson-context` (LessonModal banner). The response also carries `examAllEntriesByStudent` (every loaded season incl. No-Exam markers, `noExam:true`) so **switching the dialog's Exam select rebuilds the rows from THAT season's own saved records** (`examStateForType`) instead of carrying the current rows across; a hint under the select says "WA3 is over for X — switch to EOY to enter the next exam" when the student's displayed season is done. `examEntriesByStudent`/`examsByStudent`/`examAssessmentByStudent` are built from the DISPLAY season only. ⚠ The hub ⚠ exam-info-gap criteria (`checkExamInfoStatus`/`seasonSatisfyingTypes`) are deliberately still keyed to the ACTIVE season — a complete EOY entry during WA3 does not clear a missing-WA3 gap.
- **📷 From photo topic extraction** (2026-08-08) — button beside "Topics tested" in the exam dialog: upload the student's photo (school "topics tested" list / timetable / teacher message) → `/api/admin-schedule/extract-exam-topics` (admin auth, `claude-sonnet-5`) maps it onto the SAME canonical list `getExamTopicsForSubject()` feeds the picker, and the returned topics are auto-ticked (additive merge — existing ticks kept). Umbrella school names expand ("Sequences & Series" → APGP + Series and Sequences). Model output is gated server-side by `lib/exam-topic-extract.ts` (`flattenExamTopics` + `parseExtractionResponse`, unit-tested) — only exact canonical names survive. Bonus fills, both only when currently empty: exam date (single-paper rows only) and a ≤200-char caveat note ("Integration up to 10.1.6 only"). Client downscales to ≤1400px JPEG (`photoToJpegDataUrl`, ~200-400KB — far under Vercel's 4.5MB cap); HEIC-on-Chrome decode failure toasts a "try a screenshot" error (Safari decodes HEIC fine). One hidden file input serves all rows (`photoRowRef` carries the row index). **The photo is kept** (2026-08-22): the route also `put`s the downscaled image to Vercel Blob `exam-photos/<uuid>.<ext>` (public, random suffix) and returns `photoUrl`; the dialog holds it on the row (`ExamSubjectRow.photoUrl`, P1 row in a split) and `set-exams` stores it as a trailing `📷 <url>` line in **Exam Notes** — no new Airtable field; `lib/exam-notes-markers.ts` (`encodeExamNotes`/`decodeExamNotes`, unit-tested) owns both the `~|` approx prefix and this suffix, and the schedule route + `lesson-context` decode through it so the URL never leaks into the notes text. The dialog renders `🖼 View original photo` (new tab) + ✕ "Forget this photo" (`ExamPhotoLink`). Upload failure is non-fatal (topics still return, `photoUrl:null`); a row with only a photo counts as active in `set-exams` reconciliation. Blobs are never garbage-collected — forgetting a photo only drops the marker.
- **📄 Worksheets section** (2026-09-05) — in the Exam tab, between the exam rows and "Also save for classmates": revision worksheets Adrian plans for the student ahead of the exam, typed by topic. Each row = ☐ in front (**given** to the student) · topic · `☐ done` (**completed**, only enabled once given; unticking given clears done) · ✕ delete (confirm). Stored in **Supabase `student_worksheets`** (`student_id` = Airtable rec id, `exam_type` = the dialog's type at add time, `given`/`completed` + `_at` stamps; RLS on, service-role only — same posture as `admin_todos`, NO Airtable field). Route `/api/admin-schedule/worksheets` (GET `?studentId=` / POST `{studentId, topic, examType?}` / PATCH `{id, given?, completed?}` / DELETE `{id}`, `verifyAdminAuth`). The panel (`ExamWorksheetsPanel` in the schedule page) is self-contained and **writes immediately** (optimistic, rolls back on failure) — it is NOT part of the exam Save, and "Also save for classmates" does not copy worksheets. Rows from other seasons still show, tagged with their exam type. Silent (no Telegram).
...
| `/api/admin-schedule/worksheets` | GET/POST/PATCH/DELETE | Per-student revision worksheets (Supabase `student_worksheets`) for the exam dialog's 📄 Worksheets section |
...
- Hardcoded windows in `lib/exam-season.ts` (`EXAM_WINDOWS`): WA1 02-01→03-15, WA2 04-01→05-29, WA3 07-01→09-05, EOY 09-20→11-10 (MM-DD, SGT)
- Season handover helpers (2026-08-22, all unit-tested): `nextExamType(active)` (WA3→EOY; in a gap → the next window to start; EOY→null), `previousExamType()`, `scheduleExamTypes(active)` (what the schedule/lesson-context fetch), `pickDisplaySeason(records, active, today)` → `{examType, upcoming}` per student, `defaultEditExamType(pick, active)`, `levelSpecificExamType(type, level)` (Sec4/JC2 Prelim, JC1 Promo). See the "Season handover" chip-feature note under /admin/schedule.
...
| `lib/exam-notes-markers.ts` | `encodeExamNotes()`/`decodeExamNotes()` — `~|` approx prefix + `📷 <url>` photo suffix in Exam Notes |
