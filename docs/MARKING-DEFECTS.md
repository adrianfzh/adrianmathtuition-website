...
| E16 | a scheme DERIVED from a student's page reads outranks the bank for every later marking of that paper (`gce 2025 em p1/p2` carry unvetted derived rows from 8 Sep; they happen to agree with the bank — the 10 Sep AM P1 poisoning was the same shape and did not) | rule | stored rows win before the bank is queried, whatever their status | agent K2: approved scheme > bank > derived/extracted row > page brackets; a disagreement between a derived row and the bank Telegrams one line | 🔧 in progress |
