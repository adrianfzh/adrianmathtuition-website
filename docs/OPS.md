...
| `exam-extraction-cc1..6` | `*/5 * * * *` (was `*/30`; cadence pre-set 2026-09-02) | **this MacBook Pro** (registered on Adrian's main account — the machine the rows above call "A") | cc1–cc3 ✅ enabled (`*/5`), cc4 enabled 8 Sep 2026 00:27 but STARVED, cc5–cc6 ⏸ (state 8 Sep 2026 — was all-disabled 2 Sep 14:15) | the main paper→bank producer (40 of ~57 DONE on 2026-09-02); thin shims, law = Supabase `extraction_worker_prompt` id `exam-extraction`. The runner never double-starts a task and launches a missed fire the moment a run ends, so `*/5` ≈ continuous. **The app dispatches at most 3 scheduled-task runs at once** (`main.log` `[CCDScheduledTasks] … global_limit (active=3, limit=3)`, `per_task_limit (limit=1)`; undocumented, no setting) and re-checks due tasks every minute in registry order cc1→cc2→cc3→cc4, so a finished `*/5` task is already overdue and refills its own slot before cc4 is reached: a 4th enabled worker runs only on its enable-run (cc4: 1 run in 7 h on 8 Sep, 383 `global_limit` skips). Budget note (measured 2026-09-02): 6 workers ≈ 300M context tokens/h and hit the 5-hour session limit 3× in a morning; a run that hits it PARKS with its claim held until the reset (85-min and 4-hour parks measured). 3 workers at `*/5` ≈ 175M/h is the no-park size — drop cc4–cc6 if limit hits recur. Does NOT stamp `job_runs` (the law has no stamp step) |
