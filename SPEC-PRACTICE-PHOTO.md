...
day.* — and the science hand-in page says which day a paper will queue for before the upload.
...
| The student's pages | `app/practice/photo-client.tsx`, `todo-list.tsx` (+ `remove-sheet-button.tsx`), `app/science/submit/page.tsx` + `submit-client.tsx` (`queueNotice`), `app/science/science-papers.tsx` (+ `science-queue-remove.tsx`) |
