...
| **Grounding the chat solver on the bank** — a photographed past-paper question is looked up by the paper header the page prints and answered against the bank's own key (spec 9 Sep 2026, **built 10 Sep on the Telegram photo path**; web chat's image path not yet; bot repo `lib/solver-grounding.js`) | [`SPEC-SOLVER-BANK-GROUNDING.md`](SPEC-SOLVER-BANK-GROUNDING.md) |
