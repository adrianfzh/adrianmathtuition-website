...
- **🔬 Physics practice (2026-09-02, admin-preview only)** — the science bank in the SEPARATE Supabase project (SPEC-SUBJECTS.md Part 1; env `SUPABASE_URL_SCIENCE` + `SUPABASE_SERVICE_KEY_SCIENCE`, now in Vercel prod+preview) joins `/app/practice` as level key `PHY`: `lib/science-levels.ts` (pure, tested — level keys, `scienceLevelsFor` access, MCQ parse/grade, mastery math) + `lib/science-bank.ts` (server: eligibility twin of `practice_next` over PostgREST — not quarantined, AI-only-if-verified, has answer/solution + stem, **images only with a clean watermark scan → text-only for now, ≈2.1k MCQ + ≈100 structured**; counts cached 10 min). Every practice route branches on `isScienceLevel`/`subject:'physics'` (topics/overview/subgroups/next/solution/grade/timed-set); `lib/practice.practiceLevelAllowed`/`practiceLevelsFor` are the async gates. MCQ = A–D chips, graded deterministically (`attempted_via:'portal-mcq'`, cap-exempt); structured = Opus with the physics framing in `practice-grade-prompt` (units, s.f., formula→substitution→answer). **Attempt rows store `question_id: null` + `marking_json.science{subject,questionId}`** — the FK points at the math bank — so science mastery is computed in JS (`scienceMasteryFor`). Gate: `SCIENCE_PRACTICE_OPEN_TO_STUDENTS=false` (`lib/portal-beta.sciencePracticeAccess`: admin cookie previews every science level; when opened, a student needs `Physics` in Airtable Students.Subjects — **the option does not exist yet**, create it via typecast). Health check `science-bank` probes `bank_topics`.
