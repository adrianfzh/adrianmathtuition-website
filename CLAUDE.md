...
| **Portal v2 plan** — subjects on every paper (AM/EM pills, per-subject tiles, the subject gate), Practice as the to-do list, the finder's similarity tiers, the Notebook's fading mistakes list, Practice Again handing back its questions (agreed 6 Sep 2026, **switched OFF 8 Sep — sheet only**, `PRACTICE_AGAIN_HANDS_BACK_QUESTIONS`; only the paper sort is done) | [`SPEC-PORTAL-V2.md`](SPEC-PORTAL-V2.md) |
...
| **Science paper marking** — the three subject brains, scheme grounding, the calibration gate, the Science tab + free student marking (10 Sep 2026), the teacher's-mark calibration row, the Cambridge booklet pass | [`SPEC-SCIENCE-MARKING.md`](SPEC-SCIENCE-MARKING.md) + `docs/MARKING.md` §The Science tab |
| **Essay marking — English + 中文** (`/app/languages/*`, `/api/portal/essays`, `/api/admin/essays`, `/admin/essays`, `essay_runs`, `lib/essay-*.ts`, `data/rubrics/`, bot `ai/essay-marker.js` + `lib/essay-report.js`; **E1 BUILT 13 Sep 2026, preview identity + Adrian only**): feedback first, a band RANGE, never a point mark; the sixteen fixed error codes; calibration by consistency + ranking against each teacher + anchors, not ±2 of Adrian; the third Languages family | [`SPEC-ESSAY-MARKING.md`](SPEC-ESSAY-MARKING.md) |
| **My Notebook v2** — the student's own book + assistant (agreed 10–11 Sep 2026): opt-in switches in Settings (`portal_accounts.prefs`, `lib/portal-prefs.ts`), Save an answer, the exam countdown at the top of Home (BUILT 11 Sep), search, OCR + auto-tag, a line from Adrian, private notes, the one-stream UI, Before the paper, the Apple Preview iPad loop, push-a-page, the NotebookLM takes; build order in §13 | [`SPEC-NOTEBOOK-V2.md`](SPEC-NOTEBOOK-V2.md) |
...
| **Grounding the chat solver on the bank** — a photographed past-paper question is looked up by the paper header the page prints and answered against the bank's own key (spec 9 Sep 2026, **built 10 Sep on the Telegram AND web-chat photo paths**; bot repo `lib/solver-grounding.js`) | [`SPEC-SOLVER-BANK-GROUNDING.md`](SPEC-SOLVER-BANK-GROUNDING.md) |
...
| **GCE-format papers** — the six `GCE-*` blueprint entries, `--gce` derivation, the O-Level/A-Level shape on `/app/print` + `/admin/prelim-builder`, the SEAB-style NEW-question generator (`scripts/gce-paper/`, plan-billed Claude Code agents, never the API — the method is the `gce-paper` skill), and **Set papers** — `publish.mjs` files a generated paper in the bank as the fourth Print-a-paper preset (`lib/print-sets.ts`; A Math Set 1 live since 9 Sep 2026) | [`docs/GCE-PAPER.md`](docs/GCE-PAPER.md) |
| **Twins** — our own question for every school question we serve: what a twin is, the author → novelty → structure → blind-solve → moderate → figure gates, `twin_of`, the per-topic flip, the batch task (spec 11 Sep 2026, not built) | [`SPEC-TWINS.md`](SPEC-TWINS.md) |
| **Running a batch of fixes with agents** — triage into bins first, ≤4 agents staggered in their own clones, STATUS.txt every 15 min, verify on the golden bench never by a whole-paper re-mark, the `docs/MARKING-DEFECTS.md` ledger, 2–3 bot deploys an evening, each session in its own clone | [`docs/FANOUT.md`](docs/FANOUT.md) |
| **Other people's exam questions** — what may be served to a student, what stays grounding-only (national `school='GCE'` rows), the six rules (no public pages, no whole papers, keep the source, takedown, lawful access), the twins direction | [`docs/CONTENT-POLICY.md`](docs/CONTENT-POLICY.md) |
...
- `admin/desk/page.tsx` — 🖊 **the marking desk** (2026-09-02, the hub's marking tile): one queue in four DERIVED lanes (needs a student · marked, no sheet yet · in process · released — `lib/desk-state.ts`, pure/tested; opens on In process, work lanes OLDEST first, the released lane is LABELLED "Completed" and stays newest first — `orderLane`/`LANE_LABEL`, Adrian 7 Sep 2026) + one detail view (cover + every marked page with Agree/Override on EVERY question | the self-study sheet rendered via pdf.js + re-queue + diagnosis) ending in **Approve & release** (→ `release-with-sheet`). **8 Sep 2026: 🔁 Re-mark this page on every marked page (the other pages keep their marking — the bot seeds their reads from the stored results, the Mac reads one page, the sheet is REVISED for what changed) and ✏️ Revise this sheet (a note → `sheet-jobs {action:'revise'}` → the worker changes only what it names)** → `docs/MARKING.md` §Compartmentalised marking. **11 Sep 2026: every marked row also carries 📘 Sheet (this paper alone) and 🔁 Re-mark (the WHOLE paper through the 🌙 queue — `mark-paper {phase:'enqueue', remark:true}` with no pages, so under 🖥 Mac plan only it goes to a Mac slot, never the API).** **10 Sep 2026: tick ≥ 2 of a student's same-maths papers in a lane → 📘 ONE merged Practice Again sheet** (`POST sheet-jobs {runIds}` → `queueSheetBatch`; `sheet_jobs.run_ids` / `portal_assignments.source_run_ids`, primary run kept in `run_id` / `source_run_id`; the worker's batch rules are `WORKER_PROMPT.md` §1e) → `docs/MARKING.md` §Practice Again batches. No writes of its own; sheets auto-queue via `lib/sheet-queue.ts` → `docs/MARKING.md` §The marking desk
...
- `admin/send-page/page.tsx` — 📖 **Send a page** (11 Sep 2026, SPEC-NOTEBOOK-V2 §12, Class Notebook's move): one PDF or picture + title + a line → every chosen student (everyone / by level / picked names) gets a read-only `portal_assignments` row of **kind `page`** (never "to do" — `lib/assignments isPage` keeps it out of pending counts and the Home card), a 📖 band in My Notebook and a "Pages from Adrian" section on `/app/assignments`, a Telegram line where linked + a web push. ONE file for all under `pages/<uuid>.<ext>` in the student-files bucket (`pageKey`), which `/api/files` opens to any logged-in student. Routes `api/admin/send-page` (GET roster · POST send; health-check probes the 401) + `send-page/upload-token`
...
- `app/*` — **Student Portal** (`/app` dashboard, `practice`, `learn`, `notes`, `reference`, `settings`). `app/plan` = redirect to `/app/my-notes` since 2026-08-28 — the "My Plan" bands (SPEC-REVISION-PLAN.md; `lib/plan.ts` pure/tested over `lib/notebook-data.ts`) now render inside **My Notebook**. **Marking-only beta since 2026-08-21** (`lib/portal-beta.ts` `MARKING_ONLY_BETA`): students see only Home / Submit / Marked (+ Settings); practice/learn/notes/reference routes bounce students to `/app`, Adrian's admin cookie sees everything. `app/marking/page.tsx` = the student's own released marked scripts (**17 Sep 2026: A Math | E Math tabs, one compact row per paper, the sheet as one coloured done / not-done line, "compulsory" gone** → `docs/MARKING.md` §The simpler list); `app/submit/page.tsx` = phone-first paper hand-in (**Pencil ink comes through since 11 Sep 2026** — a PDF a student wrote on in Preview on an iPad rasterises WITH its annotations: `lib/pdf-pages.ts` renders the annotation layer, and a page carrying an annotation without the Print flag renders with intent 'any' so nothing written is dropped, SPEC-NOTEBOOK-V2 §11; **1 EXAM paper per student per SGT day, shared with the bot's /handin** — `countHandinsToday` in `lib/portal-submit-limit.ts`; since 7 Sep 2026 Practice Again / From Adrian sheets (`assignment_id`) and printed bank papers (`generated_paper_id`) neither check nor spend it, on both surfaces; spread-split + Blob client tokens → a ⏳ pending run **auto-queued into the bot's 🌙 marking queue** → marked → **auto-released** to the student (bot calls `mark-triage {action:'release', auto:true}`; margin-tick degradation is the only hold) — Adrian's Telegram says "✅ Released"; since 9 Sep 2026 evening papers Adrian uploads himself release the same way once tagged — only an untagged paper waits) → `docs/MARKING.md`
- `app/marking/[id]/page.tsx` — **one marked paper, in the app** (7 Sep 2026): the "Where your marks went" cover first (`GET /api/portal/marking-cover?run=` — `lib/front-page-build.ts`, rendered once per release, cached at `runs/<id>/cover-<release>.png`), the marked pages (`fileHref`), clip-to-notebook, the Practice Again sheet's status + Hand in, or the **Request Practice Again** button (`PracticeAgainRequest.tsx` → `POST /api/portal/practice-again/request`, 8 Sep 2026 — a sheet exists only on request), then the PDF as a download. The Papers list's button opens this, not the PDF. Paper names shown to students come from `lib/paper-display-name.ts` ("A Math · GCE 2022 · Paper 1"; `StudentPaper.rawName` keeps Adrian's typed name). Student-facing copy says "app", never "portal"; the tour is one screen (`lib/portal-tour.ts`); `app/error.tsx` + `global-error.tsx` report to Sentry and show a calm retry.
- `app/science/*` — 🧪 **the Science tab** (10 Sep 2026): Math | Science switcher in the app shell (`components/PortalTabs.tsx` `FamilySwitch`), its own bottom menu Home · Hand in · Papers, free physics/chemistry/biology marking for every student with the disclaimer, optional mark-scheme upload, its own daily slot, no Practice Again; the truth comes back as **"Your teacher's mark"** on the paper page (`/api/portal/science-truth` → one `calibration_results` row). **11 Sep 2026 (Adrian: "put in the disclaimer, don't open it yet … keep things ready so that we can release at the moment's notice"):** the release is a SWITCH, not a deploy — 🧪 "Science tab for students" on `/admin/mark-paper` beside Mac plan only (Airtable `Settings` row `science_marking_open`, `lib/marking-settings.ts`; `scienceMarkingOpen()` reads it, 30 s cache, unreadable = closed; the code flag `SCIENCE_MARKING_OPEN_TO_STUDENTS` stays a hard override; `SCIENCE_PREVIEW_IDENTITIES` = the demo student, portal-teste@example.com, who sees the tab while it is closed so Adrian can test as a student). A science paper page leads with the FEEDBACK: no score pill in the header, the disclaimer says the total is an estimate, the cover + marked pages come next, then an "Our estimate" card, the teacher's-mark card, and **"Was this marking useful?"** (`marking/ScienceUseful.tsx` → `POST /api/portal/event {kind:'science:feedback', detail:{runId, useful, note}}`, `lib/science-feedback.ts` pure/tested — one Telegram line to the marking topic per tap, a `portal_event_log` row; the student's OPINION, never the truth signal). The list card says `est. 31/40`. Flag `SCIENCE_MARKING_OPEN_TO_STUDENTS` → `docs/MARKING.md` §The Science tab + `SPEC-SCIENCE-MARKING.md` §Decision 10 Sep 2026
- `app/languages/*` — ✍️ **the Languages family** (SPEC-ESSAY-MARKING.md, E1 built 13 Sep 2026): Math | Science | Languages switcher, its own menu Home · Hand in · Essays; `submit` = typed essay + the question, `[id]` = the report (the student's text with each slip marked in place, three habits, task, upgrades, then the band RANGE — never a mark), `essays` = the list. Door `essayMarkingOpen()` (`ESSAY_MARKING_OPEN_TO_STUDENTS=false`; the demo student + Adrian's cookie see it). `admin/essays/page.tsx` = every essay, HELD reasons. Routes `portal/essays` (session) + `admin/essays` (list + the calibration harness's hand-in); one door for both in `lib/essay-submit.ts`
...
- `app/exam-countdown.tsx` — Home **"Next exam" countdown** (2026-09-02; **11 Sep 2026: a student's own switch** — Settings → "Show my exam countdown at the top of Home", `prefs.exam_countdown` off by default, renders ABOVE everything via the `ExamCountdownTop` island in `studentMode` (no timed-set door, no topic links); `exam-countdown-notice.tsx` = the one-time Home notice while `examCountdownNoticeDue` and an exam is upcoming, either button sets `exam_countdown_notice_seen`; `settings/PrefToggle.tsx` is the generic switch both prefs use): Airtable `Exams` rows dated today→+120d (fetched inside `getDashboardData`'s existing batch, `lib/portal-exams.ts` pure/tested — `~|` approx flag honoured, Exam Notes text + results never surface), nearest 3 rows, tested-topic chips → `/app/practice?level=&topic=`, ⏱ link → timed set on those topics. Hidden at zero; strangers (no Airtable record) never see it. **Student-hidden until `EXAM_PREP_OPEN_TO_STUDENTS` flips** (same flag as the timed set — the card's last row is the timed-set door).
...
- **Student requests — RETIRED 6 Sep 2026.** `/app/requests`, `/admin/requests`, their routes and `lib/requests|portal-requests|request-draft` were deleted (Engine A drafts with no diagnosis; Find a question, the Practice list and Print a paper cover every real ask). The `portal_requests` table stays for export / delete-account / retention only.
- `app/my-notes/*` — **"My Notebook"** (2026-08-28, merged tab; nav label renamed from My Notes). **11 Sep 2026: ONE STREAM** (SPEC-NOTEBOOK-V2 §9, Adrian: "build this") — `page.tsx` builds every item with `lib/notebook-stream.ts buildStreamItems` (mistake · saved answer · photo · clipping · page from Adrian · keeps-coming-up skill; pure/tested), `stream.tsx` renders the search box (client-side `filterStream` over each item's haystack incl. a photo's OCR text — search v1), the chips (All · Mistakes · Saved · Photos · From Adrian), ➕ Add a photo beside the search, and one newest-first list; a saved answer or a mistake opens inline (rename/delete, Corrected, practice links), a photo opens the gallery's `NoteLightbox`, a page opens its route; `?open=<item id>` lands open — the Home 🔁 resurface card's door (`lib/resurface.ts` `pickResurface`, deterministic per student per SGT day, live mistakes two days in three; opt-in `prefs.resurface`, Settings → "One thing a day from my notebook"; `app/resurface-card.tsx` island + "Done for today" in localStorage). **Also 11 Sep 2026:** ✍️ **My notes** (§8 — "✍️ Write" beside the search box; `notebook_private_notes`, RLS with no policies, read ONLY by the Notebook loader and `/api/portal/notebook/private-notes`; no admin page, no view-as door, no AI feature, no digest; in the student's export, deleted with the account; lib/notebook-private-notes.ts pure/tested), 📝 **Before the paper** (§4 — one card per Airtable Exam within `BEFORE_PAPER_DAYS` = 5 opening `/app/my-notes/before/[examId]`: the book narrowed to the tested topics — live mistakes, keeps-coming-up, saved answers, photos/clippings — plus the formulas met there; `lib/before-paper.ts` `topicMatches` is the filing rule, pure/tested; no exam keyed → no card; no switch), 📐 **My formulas** (§5 — `/app/my-notes/formulas`, the formulas for the topics the student has MET, `lib/formula-sheet.ts` pure/tested over `formula_ref` via the teaching-knowledge layer (`lib/formula-sheet-store.ts` — formula_ref files by AREA, so topics map to areas first) + the `/formulas/*` page per topic + ⚠ marks from live mistakes; ONE component `my-notes/formula-sheet.tsx` serves both pages), and `lib/notebook-load.ts` = the ONE loader the three Notebook pages share. The bands below describe the DATA the stream shows: ① **Your mistakes** (6 Sep 2026, SPEC-PORTAL-V2 §6 — `notebook_mistakes`, one row per student × mistake pattern, `lib/notebook-mistakes.ts` pure/tested + `lib/notebook-mistakes-store.ts`; fed by mark-triage's release action and the practice grader, fail-soft; Still happening → Getting better → Fixed, "Corrected" via `POST /api/portal/notebook/mistakes`; "This week's focus" + Home's focus card were removed with it — `lib/plan.ts` only serves `/api/portal/plan` now; **①b Keeps coming up — 10 Sep 2026, OPT-IN, BY SKILL**: Settings → "Show skills I keep asking about" writes `portal_accounts.prefs.ask_signal` through `POST /api/portal/settings {prefs}` (whitelist + merge in `lib/portal-prefs.ts`); when on, the bank sub-skills (`subgroups.name`) the student asked the bot about ≥ 3 times in a fortnight become lines — the bot files every LINKED ask under a sub-skill into Supabase `ask_skills` (bot `lib/ask-skill.js` on the web + Telegram paths after the Topic PATCH, one Haiku-class `classifySubgroup` call; `scripts/backfill-ask-skills.js` filled the log's last month), the app folds the rows at render time (`lib/ask-signal.ts` pure/tested + `lib/ask-signal-store.ts`), an ask the bot could not file falls back to its topic line — Keeps coming up → Coming up less → gone; an ask is not a verdict, so no Corrected button, no evidence, never a mistakes row) **①c 💾 Saved answers — 11 Sep 2026, OPT-IN** (Settings → "Save answers to my notebook", `prefs.save_answers`): a 💾 button under every fresh Ask answer (`lib/chat-solver attachSaveButton`) → `POST /api/portal/notebook/saves {messageId, chatId}` — the chat token must match the conversation's `chat_key` (the feedback endpoint's anti-spoof rule), the question is the user message before it, topic + skill come from the nearest `ask_skills` row (`lib/notebook-saves.ts` pure/tested), rows in `notebook_saves` (RLS, service key, identity-filtered), a second tap returns the same row; the band `my-notes/saved-answers.tsx` groups by topic, tags by skill, renders the answer with the Ask tab's own `renderToElement` + KaTeX, rename (PATCH) / delete (DELETE). Restored (older) chat turns and the Telegram answer button are not yet wired. Health-check `notebook-saves` probes the 401. ② ~~Questions to retry~~ — **DROPPED 10 Sep 2026** (Adrian, after the numbers: 197 live entries across 11 students, 29 with a bank twin, 0 attempts ever; `notebook_entries` rows still accrue at release for export/retention, `lib/notebook.ts retryOrder` stays tested, the band + `RetryCard` are gone) ③ ✂️ clippings gallery from marked papers (`portal_notes`, Blob `portal-notes/`; clip UI lives on `/app/marking`)
- `app/reschedule/*`, `app/print/*` (open to ALL students since 2026-08-28; four presets since 9 Sep 2026 — mock / my topics / weak spots / **📚 Set papers**, the bank's `AdrianMath · Set n` rows via `lib/print-sets.ts`), practice `?qid=` deep links + 📷/🔍 finder (`/api/portal/similar|generate`, 5-gen/day cap in `portal_generation_log`, bot endpoints `/api/portal-similar|generate`) → docs/SCHEDULE.md + SPEC-PRINT-PAPER.md
...
- `privacy/page.tsx` — **the SHORT parent-facing notice** (11 Sep 2026, Adrian: "use the short version from now on"): no vendor, region or architecture named, protection in two sentences, the tuition address as the DPO contact; the internal processor list lives only at the foot of `docs/PRIVACY-DRAFT-2026-09.md` and is never published. `POLICY_VERSION` (`lib/portal-consent.ts`) is bumped only when what is collected or why changes; existing consents keep their version. Nothing on `/privacy` or `/terms` mentions past papers
...
- `admin/marking-settings/route.ts` — the marking switches (11 Sep 2026): GET `{macOnly, scienceOpen}` · POST `{macOnly}` or `{scienceOpen}` flips one (Telegram line to the marking topic). 🧪 `scienceOpen` = the Science tab's release switch (Airtable `Settings` row `science_marking_open`, read by `lib/portal-beta scienceMarkingOpen()`). 🖥 **Mac plan only**: `lib/marking-settings.ts` stores it as the Airtable `Settings` row `marking_mac_only` (the auto-release switch's shape) and the BOT reads that row every queue tick (bot `lib/marking-settings.js`) — ON = the worker marks nothing itself (no full-price API, no batch, no takeover), every paper waits for a Mac slot, ⚡ Mark now / ☁️ Batch now rows go to the Mac too; the card lives on `/admin/mark-paper` above Recent marked papers; health-check probes the 401
- `admin/slot-accounts/route.ts` — ⏻ **the Mac slots' per-account switches** (13 Sep 2026): GET `{accounts, off}` · POST `{email, on}`; one Airtable `Settings` row `slot_accounts` (`lib/slot-accounts.ts` pure/tested + `slot-accounts-store.ts`), keyed like the workers' plan-limit files; every marking/sheet slot (bot `worker/plan-marking/run.sh`, `scripts/sheet-worker/run.sh`) asks it before claiming and fails open; the card sits on `/admin/mark-paper` under the science switch; health-check probes the 401
...
- `portal/notebook/private-notes/route.ts` — ✍️ **the student's private notes** (11 Sep 2026, SPEC-NOTEBOOK-V2 §8): GET / POST `{body}` / PATCH `{id, body}` / DELETE `{id}` on `notebook_private_notes`, keyed ONLY by the session's own portal identity — no admin or bearer path exists on purpose; health-check `notebook-private-notes` probes the 401
- `portal/telegram-link/route.ts` — **in-app Telegram linking** (8 Sep 2026, Adrian: "put the generation of token in the app itself, and when they link, it disappears"): GET (session) mints a signed 60-min `https://t.me/AdrianMathBot?start=tg_…` deep link (`lib/telegram-link.ts`, pure/tested — uuid + expiry + HMAC under `BOT_INTERNAL_SECRET`, fits Telegram's 64-char start param), `?check=1` → `{linked}`; POST (bot, Bearer `BOT_INTERNAL_SECRET`) `{token, chatId}` binds `portal_accounts.telegram_chat_id` and fills Airtable `Student Telegram ID` only when empty (409 when the account is linked to a different chat, 410 expired). `components/TelegramLinkCard.tsx` = the Home nudge (students only, rendered only when `lib/telegram-link-state.ts` `ensureTelegramLinked` says `unlinked` — it inherits an existing Airtable id into the account once, and `unknown` hides the card) + the Settings card (✓ Linked · Unlink, or Link + a folded paste-a-chat-ID fallback). The bot side is `lib/portal-link.js` + the `/start tg_…` payload branch in `handlers/commands.js`. Health-check `telegram-link` probes the POST's 401
- `portal/event/route.ts` — student-side counterpart (POST `{kind}`, allow-list `marking:view`/`marking:open`, the install/push kinds, and **`submit:failed` + `detail`** — a hand-in that failed on the phone after every retry (`lib/submit-failure.ts`, 7 Sep 2026: stored in `portal_event_log.detail`, one Telegram line per student per hour to the marking topic, the hub's red "Hand-in failed on a phone (24h)" card via `lib/portal-activity.ts` `failedHandins`), writes `portal_event_log`); fired by `/app/marking`'s `MarkingBeacon.tsx` → `PORTAL.md` §Activity
...
- `portal/practice-again/request/route.ts` — **the student's Practice Again door** (8 Sep 2026): POST `{runId}` on their own released run → `queueSheetJob(…, {requestedBy:'student'})`; the sheet goes out by itself once written and gated; 401 anon (health-check `practice-again-request`) → `docs/MARKING.md` §Practice Again on request
- `cron/practice-again-reminders/route.ts` — daily 09:00 SGT nag for COMPULSORY sheets (`portal_assignments.required_at`, set when Adrian releases a sheet he queued himself): day 3, then weekly, ×4, Telegram + push (`lib/practice-again-reminders.ts`, tested; `lib/student-recipient.ts`) → `docs/OPS.md`
...
- `cron/scan-inbox/route.ts` — 📠 **the ScanSnap watcher** (7 Sep 2026, every 15 min since 9 Sep): a PDF landing in the app folder's `/Scans` is read (cover → student, paper), named by the convention, tagged, put in the 🌙 queue and renamed; non-exam scans are left alone. Ledger `scan_inbox`; pure pieces `lib/scan-inbox.ts`. **The same tick runs the 🏷 auto-tag sweep** (`lib/auto-tag-sweep.ts` + `lib/auto-tag.ts`, 7 Sep 2026): untagged runs ≤ 14 days are tagged from the typed name, or from the first page (cover reader, once) when the name fits two students — never a guess; tagged+marked runs with no sheet job get one queued → `docs/MARKING.md` §The ScanSnap watcher
- `cron/extraction-inbox/route.ts` — 📥 **the extraction inbox watcher** (8 Sep 2026, every 10 min): a `.docx`/`.pdf` dropped in the app folder's `/Extraction Inbox` is hashed, uploaded to the private `paper-library` bucket and queued as a `paper_library` row (`kind='source'`, `status='queued'`; a name the fleet convention can't file is `flagged`, never lost; known bytes are rejected, not queued twice). Workers claim through `admin/extraction-queue` (POST `claim` → atomic `claim_extraction_paper` RPC with a 3-hour lease + a signed download URL; `finish` / `requeue`) — the replacement for renaming files in the iCloud `papers/` folder. **Cut over 10 Sep 2026:** the fleet law's step 0 claims from this queue; the in-app task `inbox-extract` (queue-only; **three times a day, 08:00 / 14:00 / 22:00 SGT, plus Run now** — Adrian, 11 Sep 2026: "it's actually a pretty infrequent thing"; an empty run is one claim request and stops) banks a dropped paper with from-scratch solutions. **The same tick files the MARKER's row** (`kind='questions'|'solutions'`) over the same object and re-marks recent runs that were marked without that paper, and **a PDF with no paper number in its name is cut at its cover pages first** (`lib/paper-book-split*.ts`, one `claude-sonnet-5` document read; a combined TYS book becomes `AM GCE 2025 Paper 1.pdf` + `Paper 2.pdf`, each queued and filed on its own — Adrian: "should not have to split myself"). Pure pieces + tests `lib/extraction-inbox.ts`, `lib/paper-book-split.ts` → [`docs/EXTRACTION-QUEUE.md`](docs/EXTRACTION-QUEUE.md)
...
- `bot/worksheet/route.ts` — **worksheet-on-demand** (auth: `x-render-secret`). POST `{level, topic, tier?, count?≤12, answers?}` → house-style A4 PDF in Singapore Storage (`practice_worksheets/bot/…` via `lib/bank-pdf-store`, Blob fallback — the US Blob upload was half of every warm build until 7 Sep 2026), `{url, store, title, count, questionIds, filename, timings}`. Same deterministic daily draw + eligibility gate as the kiosk (`lib/kiosk-pool` + `lib/kiosk-draw`), so it can never leak worked solutions or originating-school metadata. `{dry:true, level, topic}` → `{ok, poolSize}`, the health-check probe. Renderer: `lib/render-bot-worksheet.ts`; request logic + tests: `lib/bot-worksheet.ts` → `docs/KIOSK.md`
