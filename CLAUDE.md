...
- **Topic timeline: planned next-lesson topic + self-healing corrections** (`/api/admin-schedule/topic-timeline`) — a row with **no Started and Current=false is a PLAN** ("next lesson's topic"; no new Airtable fields). POST actions: default advance, `plan`, `startPlanned`, `clear`, rowId edit/delete. Correction rules: advancing away from a topic **set today deletes** the mis-pick (no `24 Jul – 24 Jul` noise rows); re-picking a topic **ended today resurrects** its original row; advancing into the planned topic consumes the plan. Every write calls `invalidateScheduleStatics()` (Topic Timeline is in the statics cache — without it the chip's 📘 pill stayed stale for up to 60s). Work-tab UI: picker hidden behind ✏️ Change once a current topic exists; NEXT LESSON section with ▶ Start now.
- **⚠ exam season pill** — appears when student is missing exam date or tested topics for the active exam type
- **Exam-season "what to expect" summary** — during an active exam season each chip shows, under the pill, a compact per-subject line (`examSummaryLines()`): `WA3 · A Math · 20 Aug — Differentiation, Integration` (P1/P2 split renders `P1 …, P2 …`). Data from `examEntriesByStudent` (each entry now carries `examType`).
