...
| **GCE-format papers** — the six `GCE-*` blueprint entries, `--gce` derivation, the O-Level/A-Level shape on `/app/print` + `/admin/prelim-builder`, and the SEAB-style NEW-question generator (`scripts/gce-paper/`, plan-billed Claude Code agents, never the API) | [`docs/GCE-PAPER.md`](docs/GCE-PAPER.md) |
