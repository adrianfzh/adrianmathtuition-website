...
- `app/*` — **Student Portal** (`/app` dashboard, `practice`, `learn`, `notes`, `reference`, `settings`). `app/marking/page.tsx` = the student's own released marked scripts — the destination `mark-triage`'s release nudge links to; `app/submit/page.tsx` = phone-first paper hand-in (spread-split + Blob client tokens → a ⏳ pending run **auto-queued into the bot's 🌙 marking queue** — the marked result Telegrams Adrian with the 🖼 PDF; Release stays manual; doorbell only fires if the enqueue fails) → `docs/MARKING.md`
