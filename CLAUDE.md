...
- **🔬 Science practice — physics, chemistry, biology (2026-09-02, admin-preview only)** — the science bank in the SEPARATE Supabase project (SPEC-SUBJECTS.md Part 1; env `SUPABASE_URL_SCIENCE` + `SUPABASE_SERVICE_KEY_SCIENCE`, now in Vercel prod+preview) joins `/app/practice` as level keys `PHY` / `CHEM` / `BIO` (`SCIENCE_LEVELS` rows — one row per subject is all it takes): `lib/science-levels.ts` (pure, tested — level keys, `scienceLevelsFor` access, MCQ parse/grade, mastery math) + `lib/science-bank.ts` (server: eligibility twin of `practice_next` over PostgREST — not quarantined, AI-only-if-verified, has answer/solution + stem, **images only with a clean watermark scan → text-only for now: physics ≈2.2k, chemistry ≈3.8k, biology ≈1.3k, ≈85 % MCQ**; counts cached 10 min). Every practice route branches on `isScienceLevel`/`subject:'physics'` (topics/overview/subgroups/next/solution/grade/timed-set); `lib/practice.practiceLevelAllowed`/`practiceLevelsFor` are the async gates. MCQ = A–D chips, graded deterministically (`attempted_via:'portal-mcq'`, cap-exempt); structured = Opus with a per-subject examiner framing in `practice-grade-prompt` (physics: units/s.f./formula→substitution→answer; chemistry: balanced equations, specific observations, mole working; biology: named terms, cause→effect chains, quoted data). **Attempt rows store `question_id: null` + `marking_json.science{subject,questionId}`** — the FK points at the math bank — so science mastery is computed in JS (`scienceMasteryFor`). Gate: `SCIENCE_PRACTICE_OPEN_TO_STUDENTS=false` (`lib/portal-beta.sciencePracticeAccess`: admin cookie previews every science level; when opened, a student needs `Physics` / `Chemistry` / `Biology` in Airtable Students.Subjects — **those options do not exist yet**, create them via typecast). Health check `science-bank` probes `bank_topics`.
...
- `app/my-notes/*` — **"My Notebook"** (2026-08-28, merged tab; nav label renamed from My Notes): ① This week's focus band (`buildPlan` — same derivation as Home's card, hidden when empty) ② Questions to retry (live `notebook_entries`, `retryOrder` in `lib/notebook.ts`; bank twins deep-link `/app/practice?qid=…&from=notebook`) ③ ✂️ clippings gallery from marked papers (`portal_notes`, Blob `portal-notes/`; clip UI lives on `/app/marking`)
...
- `admin/papers/route.ts` — marked-script library (Supabase `paper_marking_runs` direct, NOT the bot proxy); GET `?days=&limit=&student=&untagged=1&subject=`, POST `{runId, studentId|null}` to tag → `docs/MARKING.md`
