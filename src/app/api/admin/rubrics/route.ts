...
import { getSupabase, getSupabaseAdmin, getSecretKey } from '@/lib/supabase';

export const runtime = 'nodejs';

// Admin view/edit of the Solo grading rubrics (see RUBRIC-SPEC.md). GET reads via
// the anon client (RLS read-all). PATCH writes via the privileged client — which
// requires SUPABASE_SECRET_KEY (or legacy SUPABASE_SERVICE_ROLE_KEY); without it,
// editing is done in Supabase directly.
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ rubrics: data || [], canEdit: !!getSecretKey() });
}
...
export async function PATCH(req: NextRequest) {
  ...
  if (!getSecretKey()) {
    return NextResponse.json(
      { error: 'In-app editing needs SUPABASE_SECRET_KEY in the environment. Until then, edit the rubric directly in the Supabase dashboard.' },
      { status: 503 });
  }
