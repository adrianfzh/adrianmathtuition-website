...
export async function POST(req: NextRequest) {
  ...
  // Tagging no longer queues a sheet (8 Sep 2026 — Practice Again is written
  // on request: the student asks from the app, or Adrian from the desk). The
  // paper's Dropbox folder still leaves /Students/_Untagged (7 Sep 2026).
  if (studentId) after(() => refileUntaggedFolder(runId));

  return NextResponse.json({ ok: true, runId, studentId: studentId || null, studentName, sheet: null });
}
