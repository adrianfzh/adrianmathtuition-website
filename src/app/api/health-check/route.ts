...
export async function GET(req: NextRequest) {
  ...
  const parallelChecks = await Promise.all([
    ...
    timed('work-ink', async () => {
      const r = await fetch(`${base}/api/portal/work/ink`, { method: 'POST', redirect: 'manual', signal: T(10000) });
      if (r.status !== 401) throw new Error(`expected 401 (auth gate), got HTTP ${r.status}`);
      return 'auth gate up';
    }),
    timed('marking-ink', async () => {
      const r = await fetch(`${base}/api/portal/marking/ink`, { method: 'POST', redirect: 'manual', signal: T(10000) });
      if (r.status !== 401) throw new Error(`expected 401 (auth gate), got HTTP ${r.status}`);
      return 'auth gate up';
    }),
    timed('marking-note', async () => {
      const r = await fetch(`${base}/api/portal/marking/note`, { method: 'POST', redirect: 'manual', signal: T(10000) });
      if (r.status !== 401) throw new Error(`expected 401 (auth gate), got HTTP ${r.status}`);
      return 'auth gate up';
    }),
    timed('marking-archive', async () => {
      const r = await fetch(`${base}/api/portal/marking/archive`, { method: 'POST', redirect: 'manual', signal: T(10000) });
      if (r.status !== 401) throw new Error(`expected 401 (auth gate), got HTTP ${r.status}`);
      return 'auth gate up';
    }),
    timed('marking-star', async () => {
      const r = await fetch(`${base}/api/portal/marking/star`, { method: 'POST', redirect: 'manual', signal: T(10000) });
      if (r.status !== 401) throw new Error(`expected 401 (auth gate), got HTTP ${r.status}`);
      return 'auth gate up';
    }),
