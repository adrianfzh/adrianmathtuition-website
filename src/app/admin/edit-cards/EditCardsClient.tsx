...
function EditorPanel({ initialCard, subgroups, allCards, level, topic, auth,
  ...
  const [deleting, setDeleting] = useState(false);
  // Right panel tab + pending source link (cleared on save)
  ...
  // Drop a bank question onto the editor → replace content with full template
  const handleBankDropOnEditor = useCallback((q: BankQuestion) => {
    ...
    // Push current content to undo history before replacing
    setContentHistory((prev) => [...prev.slice(-9), content]);
    setContent(tplContent);
    // Only overwrite title if it was empty
  ...
  // ── Image resize/align toolbar helpers ───────────────────────────────────
  function parseImgStyle(style: string): { pct: number; align: 'left'|'center'|'right' } {
    ...
    const align: 'left'|'center'|'right' =
      ...
      : style.includes('0 8px auto') ? 'right'
      : 'center';
    return { pct, align };
  }
  ...
  const doSave = useCallback(async (fields: { card_title: string; content: string; subgroup_id: number; order_index: number; is_published: boolean }) => {
    ...
    try {
      // If a bank question was dragged in this session, persist the link too
    ...
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden">
        ...
        <div
          ...
          onDragLeave={(e) => {
            // Only clear if we've actually left the wrapper
            if (e.currentTarget.contains(e.relatedTarget as Node)) return;
          ...
          onDrop={(e) => {
            ...
            try {
              const q = JSON.parse(payload) as BankQuestion;
              if (q.usage_count > 0) {
                const ok = window.confirm(`This question is already used in ${q.usage_count} card${q.usage_count > 1 ? 's' : ''}. Drop anyway and replace the current card content?`);
                if (!ok) return;
        ...
        >
          ...
          <div className="px-3 py-1 bg-slate-50 border-b border-r border-slate-200 text-xs text-slate-500 flex items-center gap-2">
            <span>Markdown + LaTeX</span>
            {pendingSourceQuestionId && <span className="text-blue-600 font-medium" title="Linked to bank question — will save on next Save">🔗 from bank</span>}
          </div>
          <textarea
            ref={textareaRef}
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <div className="px-3 py-1 bg-slate-50 border-b border-r border-slate-200 text-xs flex items-center gap-1.5">
            ...
            {!aiPreviewContent && <span className="text-slate-400 ml-auto">Click image to resize/align</span>}
          </div>
          <div
            className="flex-1 overflow-y-auto px-4 py-3 bg-white prose prose-sm max-w-none border-r border-slate-200"
            onClick={(e) => {
        ...
        {/* Right panel — tabbed: AI Assist | Bank */}
        {aiOpen && (
          <>
