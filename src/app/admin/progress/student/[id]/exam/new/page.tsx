...
export default function NewExamPage({ params }: { params: { id: string } }) {
  ...
  const [examType, setExamType] = useState<string>('WA1');
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/progress/students', { headers: { Authorization: `Bearer ${savedPw.current}` } })
      .then(r => r.json())
      .then(d => {
        const s = d.students?.find((s: any) => s.id === id);
        if (s) {
          setSubjects(s.subjects ?? []);
          setStudentName(s.name ?? '');
          if (s.subjects?.length === 1) setSubject(s.subjects[0]);
        }
      })
      .catch(() => {});
  }, [authed, id]);
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (examType === 'Custom') body.customName = customName;
      if (resultScore) body.resultScore = Number(resultScore);
      if (resultTotal) body.resultTotal = Number(resultTotal);
      if (resultGrade) body.resultGrade = resultGrade;
      if (resultNotes) body.resultNotes = resultNotes;
    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-semibold text-slate-900">New Exam</h1>
      ...
      <div className="max-w-lg mx-auto px-4 pt-5 space-y-6">
        ...
        <div>
          ...
          {examType === 'Custom' && (
            <input
              className="mt-2 w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white"
              placeholder="e.g. School Prelim, O-Level"
              value={customName}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {subjects.length > 0 ? subjects.map(s => (
              ...
              <p className="text-sm text-slate-400">No subjects assigned to student</p>
            )}
          </div>
        </div>
