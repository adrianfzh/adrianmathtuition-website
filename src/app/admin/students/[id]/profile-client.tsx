...
// The profile shell (17 Sep 2026, SPEC-STUDENT-FIRST §2): four tabs — Overview ·
// Papers · Work · Billing & slots. `papersTab` is rendered on the SERVER by
// ./papers-tab (the student's own Papers view with admin doors) and handed in
// as a node; `?tab=` opens a tab directly (the old mirror page redirects to
// ?tab=papers).
export default function StudentProfileClient({ papersTab }: { papersTab: React.ReactNode }) {
  ...
  type Tab = 'overview' | 'papers' | 'work' | 'billing';
  const TABS: Tab[] = ['overview', 'papers', 'work', 'billing'];
  const TAB_LABEL: Record<Tab, string> = { overview: '⭐ Overview', papers: '📄 Papers', work: '📬 Work', billing: '💳 Billing & slots' };
  const initialTab = (search?.get('tab') as Tab | null);
        ...
        {!loading && s && data && (
          <>
            ...
            <Section title="Profile">
              ...
              {/* The rarely-used actions live behind one "…" (17 Sep 2026, Adrian: Discontinue "in the middle like that makes it easy to misclick"). */}
              <details style={{ marginTop: 10 }}>
                <summary style={{ cursor: 'pointer', fontSize: 13, color: '#64748b', fontWeight: 600, listStyle: 'none', display: 'inline-block', padding: '6px 10px', border: '1px solid #e5e7eb', borderRadius: 10, background: '#fff' }}>… More</summary>
