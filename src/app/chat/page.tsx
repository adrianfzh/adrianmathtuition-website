...
// The solver's client core (streaming engine, KaTeX pipeline, chat-DOM
// builders) is SHARED with the portal's /app/ask tab — it lives in
// lib/chat-solver.ts. This page keeps only what is public-chat-specific:
// layout, sidebar/formula sheets, Telegram Mini App identity, the
// registration nudge, and its own scroll tracking.
import {
  BOT_API_BASE,
  type HistoryEntry,
  ...
  attachFeedbackRow as attachFeedbackRowShared,
  autoResize,
...
export default function ChatPage() {
  ...
  /* ── Add user/bot message to DOM (shared builder + this page's scroll) ── */
  const addMessageToDOM = useCallback((role: 'user' | 'bot', content: string | null, imageDataUrl?: string | null) => {
    const inner = messagesInnerRef.current;
    if (!inner) return;
    appendChatMessage(inner, role, content, imageDataUrl);
    scrollToBottom();
  }, []);
  ...
  const addStreamingMessage = useCallback((): HTMLDivElement => {
    return appendStreamingMessage(messagesInnerRef.current!);
  }, []);
  ...
  const attachFeedbackRow = useCallback((group: HTMLElement, messageId: number, existing: string | null) => {
    attachFeedbackRowShared(group, messageId, existing, {
      getChatId: () => sessionIdRef.current,
      getTgInitData: () => window.Telegram?.WebApp?.initData,
    });
  }, []);
    ...
    fetch(`${BOT_API_BASE}/api/chat/history?${historyQs}`)
      ...
      .then((data: { messages: RestoredMessage[] }) => {
        ...
        const insert = () => {
          ...
          const inner = messagesInnerRef.current;
          if (!inner) return;
          conversationHistoryRef.current = insertRestoredMessages(inner, msgs, {
            getChatId: () => sessionIdRef.current,
            getTgInitData: () => window.Telegram?.WebApp?.initData,
          });
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (!sessionIdRef.current) {
        sessionIdRef.current = 'web-' + randomChatToken();
      }
      ...
      // The shared engine (lib/chat-solver) owns the typewriter, status
      // labels, verify/correction/graph events and resume-replay polling.
      // This page supplies its scroll behavior: reveal-time autoscroll obeys
      // the user-scrolled-up flag tracked on the chat scroll container.
      ...
      const { fullText, doneMessageId } = outcome;

      // Stream ended without a done event and resume polling came up empty —
      // the engine already rendered the "connection lost" notice.
      if (!outcome.sawDone && !fullText && !outcome.gotError) {
        return;
      }
  ...
  }, [isLoading, conversationStarted, selectedFile, previewSrc, extraFiles, extraPreviews, removeImage, addMessageToDOM, addTypingToDOM, removeTypingFromDOM, addStreamingMessage, showError, attachFeedbackRow, maybeShowRegistrationNudge]);
