...
// ── Rubric-driven English prompt ────────────────────────────────────────────
// buildEnglishSystem lives here (not in rubric.ts) so it's a PURE function with
// no Supabase dependency — the app calls it with a rubric from getRubric(), and
// the grading eval imports the exact same function to test the real prompt.
