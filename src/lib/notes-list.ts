...
//
// Four kinds live in the same app folder, one parent folder each:
//   notes     → /Notes/<LEVEL>      e.g. /Notes/AM    (kiosk "Learn" + admin)
//   revision  → /Revision/<LEVEL>   e.g. /Revision/AM (kiosk "Revise" + admin)
//   practice  → /Practice/<LEVEL>   e.g. /Practice/AM (kiosk "Practice" + admin)
...
//
// Revision was admin-only from 2026-07-28; Adrian opened it to students on
// 2026-08-11 along with the new practice kind, when the kiosk became the
// three-button Learn / Revise / Practice station. Prelim stays admin-only.
//
// The two worksheet kinds differ in what precedes the questions: a `revision`
// sheet carries worked examples, a `practice` sheet carries a summary/formula
// section. Both are generated by the copy-revision-worksheet-with-different-practice skill (kinds `worked`
// and `notes` respectively) straight into these folders.
...
export type PrintableKind = 'notes' | 'revision' | 'practice' | 'prelim';

const PRINTABLE_KINDS: readonly PrintableKind[] = ['notes', 'revision', 'practice', 'prelim'];

export function isPrintableKind(v: string | null | undefined): v is PrintableKind {
  return PRINTABLE_KINDS.includes(v as PrintableKind);
}

/** The kinds the student kiosk serves — prelim is deliberately absent. */
export const KIOSK_KINDS: readonly PrintableKind[] = ['notes', 'revision', 'practice'];

export function isKioskKind(v: string | null | undefined): v is PrintableKind {
  return KIOSK_KINDS.includes(v as PrintableKind);
}
...
export function dropboxFolderFor(kind: PrintableKind, slug: string): string | null {
  ...
  if (kind === 'practice') return `Practice/${folder}`;
 ...
 * half-moved folder still lists. No other kind ever had a legacy location.
