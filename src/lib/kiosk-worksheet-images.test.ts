...
describe('flattenParts', () => {
  ...
  it('keeps stem, labels, marks, and combined answers intact (house style: marks span + proportional spacer)', () => {
    ...
    // working-space div (17mm/mark, floor 36, cap 100). Unmarked parts get neither.
    expect(text).toBe(
      'Stem here.\n\n**(a)** First. <span class="ws-mk">[2]</span>\n\n<div class="ws-sp" style="height:36mm"></div>\n\n**(b)** Second.'
    );
    expect(answer).toBe('(a) 1;  (b) 2');
  });
  it('clamps working space to 36–100mm', () => {
    ...
    expect(one).toContain('height:36mm');
    expect(big).toContain('height:100mm');
  });
