...
import { unseenByStudent, unseenLabel, isPracticeAgainName } from './unseen-handins';

const row = (o: Partial<{ id: string; student_id: string | null; paper_name: string; created_at: string; admin_viewed_at: string | null; portal_submission: unknown }>) => ({
...
describe('unseenByStudent', () => {
  ...
  it('a run Adrian opened, a paper he uploaded himself, or an untagged run does not count', () => {
    const m = unseenByStudent([
      row({ id: '1', admin_viewed_at: '2026-09-17T01:00:00Z' }),
      row({ id: '2', portal_submission: null }),
