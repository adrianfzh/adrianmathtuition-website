...
10. ✅ (2026-08-29) Advanced-tier gaps: **94 generation_requests seeded across 47 zero-advanced (level × topic) pairs** (2 each, admin-topup row shape, text-only seeds; figure-dependent topics skipped: Constructions, Graph-on-Graph-Paper, Speed-Time Graphs, EM Graphs of Functions; S2 Coordinate Geometry (Lines) had no eligible text-only seed). Also aligned `practice_topics`' advanced_count with the canonical tier CASE (lowercase `advanced`/`Bonus` now count). The nightly plan worker drains the queue; a bulk topup-bank session can chew it faster.
