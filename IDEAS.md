...
5. ✅ **Notes vetting layers 1+2 executed**: 1,127 sub-groups judged by a 5-agent fleet. **652 descriptions re-typeset to TeX and APPLIED** (backup: `subgroups_vetting_backup_20260829`; /notes renders description math server-side now). **518 renames + 113 questions await Adrian** on the sheet: https://claude.ai/code/artifact/e723be95-6bb5-4639-803c-33f5d5248598 — raw JSON in the session scratchpad vetting/ dir; a session applies approved renames with the same backup table for rollback; ⚠ renames change /notes slugs. **Leaked pipeline text cleaned in-DB 2026-08-29** (56 rows: 26 "Generation hint" tails stripped, 30 school+year+Q citations rewritten keeping the math — incl. 7 JC Binomial rows where the fleet's own rewrite had kept citations; whole-table regex sweep = 0 remaining), and the pure-leak flags reclassified (8→keep, 21→rename), so no leak questions reach Adrian. **Sheet v4** after three UX rounds: every card labels OLD (live now)/NEW (proposed), chips name concrete outcomes ("Keep OLD name", "Merge them", "Hide it") — never "keep unchanged"; flags are per-category plain questions (84 merge / 14 hide-off-syllabus / 7 S1→S2 / 4 fix-wrong-math / 3 re-file / 1 whole-topic Modulus card); taps save to localStorage only (publish = the jump-to-top bug), Sync/Hand-to-Claude publish explicitly. Remaining fleet finds: off-syllabus content (Vieta/half-angle at 4049, Taylor/integrating-factor at 9758, discriminant at 4052), whole-topic duplicates (Numbers (Prime Factorization) vs HCF/LCM; Distributions (Sampling) vs Binomial/DRV/Normal), two wrong math statements in JC concavity groups.
