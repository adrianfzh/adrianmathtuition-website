...
1. ✅ **Notes = revision-only**: topic pages open straight into worked examples; teaching stack (key facts/techniques/Q&A units + quick-revision card) moved to `/notes/<lvl>/<topic>/learn` (search anchors follow). Verdicts applied via `subgroups.visibility` ('all'/'hidden'/'ip', filtered at the loadSubgroups chokepoint): Vieta 1522 + integrating-factor 863 + Modulus 809-814 hidden; S1-461 marked 'ip' — conservatively hidden until portal accounts carry an IP flag.
